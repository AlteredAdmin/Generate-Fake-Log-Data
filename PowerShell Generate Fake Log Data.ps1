# PowerShell script to generate random fake log data with occasional custom log message
# Including messages with IP addresses, domain names, and SQL database connections

param(
    [int]$IntervalInSeconds = 5,    # Interval between log entries
    [int]$DurationInMinutes = 1,    # Duration to run the script
    [string]$LogMessage = "Special log entry", # Custom log message
    [string]$LogFilePath = "C:\path\to\your\log\file.log" # Path to the log file
)
...
while ((Get-Date) -lt $endTime) {
    ...
    if ($random.Next(0, 10) -lt 2) { # Roughly 20% chance to write the custom log message
        $message = $LogMessage
    } else {
        $message = $fakeMessages[$random.Next(0, $fakeMessages.Length)]
    }
...
Write-Host "Log generation completed."
